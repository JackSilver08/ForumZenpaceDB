(() => {
    ...
    if (!(popover instanceof HTMLElement) || !(picker instanceof HTMLElement)) {
        return;
    }
    ...
    const enableTriggers = (root = document) => {
        if (!pickerReady) {
            return;
        }

        root.querySelectorAll('[data-emoji-trigger]').forEach((trigger) => {
    ...
    const findInputForTrigger = (trigger) => {
    ...
    const openPopover = (trigger, input) => {
        syncSelection(input);
    ...
    customElements.whenDefined('emoji-picker')
        .then(() => {
            pickerReady = true;
            enableTriggers(document);
        })
        .catch(() => {
            closePopover();
        });

    ['focusin', 'input', 'keyup', 'click', 'mouseup', 'select'].forEach((eventName) => {
        document.addEventListener(eventName, (event) => {
            ...
            const input = target.closest('[data-emoji-input]');
    ...
    document.addEventListener('zenpace:emoji-refresh', (event) => {
